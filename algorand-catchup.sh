...
LAST_ROUND=$(curl -s -H "X-Algo-API-Token: ${ALGOD_ADMIN_TOKEN}" "http://${ALGOD_URL}/v2/status" | jq -r '.["last-round"]')
CURRENT_CATCHPOINT=$(curl -s -H "X-Algo-API-Token: ${ALGOD_ADMIN_TOKEN}" "http://${ALGOD_URL}/v2/status" | jq -r '.catchpoint')
...
if [ "$LAST_ROUND" -lt "20000000" ] && [[ -z "$CURRENT_CATCHPOINT" ]]; then
	...
	curl -s -X POST -H "X-Algo-API-Token: ${ALGOD_ADMIN_TOKEN}" "http://${ALGOD_URL}/v2/catchup/${LATEST_CATCHPOINT}"
else
